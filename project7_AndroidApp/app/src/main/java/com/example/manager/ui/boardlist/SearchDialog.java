...
public class SearchDialog extends DialogFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        searchButton.setOnClickListener(v -> {
            ...
            if (nameText.getEditText() == null || nameText.getEditText().getText() == null) {
                Toast.makeText(requireActivity().getApplicationContext(), "작성자를 입력해주세요.", Toast.LENGTH_SHORT).show();
                return;
            }

            // 조회할 이름을 콜백 함수로 전달
            resultListener.searchBoardsByWriter(nameText.getEditText().getText().toString());
    ...
    public interface ResultListener {
        void searchBoardsByWriter(String writer);
    }
}
